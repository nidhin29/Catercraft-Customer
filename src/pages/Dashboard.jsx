...
const Dashboard = () => {
  ...
  const fetchBookings = async () => {
    try {
      const data = await apiClient(`/User/view-bookings?customer_email=${user.email}`);
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      // Join a room specific to the customer
  ...
  const handlePayment = async (booking) => {
    ...
    try {
      const orderData = await apiClient("/booking/pay-deposit", {
        method: "POST",
        body: JSON.stringify({ booking_id: booking._id }),
      });

      const options = {
        key: "rzp_test_placeholder", // Replace with your actual key or env
        amount: orderData.data.amount,
        currency: orderData.data.currency,
        name: "CaterCraft",
        description: `Deposit for ${booking.service_id.service_name}`,
